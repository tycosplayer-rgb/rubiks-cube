...
| 入口 | URL | 说明 |
|------|-----|------|
| **推荐（非 github.io）** | https://ecmdod-ip-44-207-134-103.tunnelmole.net/ | Tunnelmole HTTPS；当前 box 在线时可用 |
...
### 金字塔（Pyraminx）
- **分段内核**：大四面体细分为小四面体，按尖轴投影分 tip / deep / bottom 三段；转尖只带 tip 内核，中层/底层内核不动（不缩小内核）
- 四**尖轴** 120° 转动（非面转）：`U层`… 棱层（9）、`U尖`… 尖角（3）、`U底`… 底层 = 远带 + 底座三角（24，`d ≤ 0.4`，含另外三尖贴纸并带动底座角循环）；尖与层相互独立；底绕 T 可带走其它三尖
- **滑动拧层**：按**当前世界坐标**判定尖（`argmax point·axis` 且 `d > TIP`），不用过期 tipPiece；对面非尖贴纸或远带 `d ≤ DEEP` → 底；否则中带 → 层
- 打乱以深层为主，偶发尖拧与底层；还原为历史逆序（保留 tip/bottom 标记）

### 十二面体（Megaminx）
- 每面 **五角星切割（star cut）**：1 中心五边形 + 5 角块 + 5 棱块（共 11 贴纸），沟槽呈五角星
- 面转 72°；按**当前面位置**选层（轴投影找出本面 11 贴纸），再按 piece id 扩到整块（约 26），相邻面转后仍正确
- **滑动拧面**：手指所在面 F 上点到棱 → 转该棱所属的**侧面**（不转 F，正面通常只见一条棱在动）；点到角 → 在两侧面中按滑动方向打分；点到中心 → 转 F。转向仅在已选面上对 ±1 步打分
- 全色模式不放大贴纸（用 polygonOffset + 几何 inset 防 z-fighting / 露核）；转动时整块灰色内核随面层旋转（无缩小）
- 打乱 / 还原为随机面转与历史逆序
